...
export default async function handleNewsLetter(req: NextApiRequest, res: NextApiResponse) {
  if (req.method === 'POST') {
    ...
    if (!response.ok) {
      return res.status(500).json({ error: 'Error al suscribirse al boletín de noticias' });
    }
